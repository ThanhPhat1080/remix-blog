      ...
      <div className="p-5">
        <div className="flex items-center justify-between py-2">
          <em className="text-xs dark:text-gray-400">{`Last updated: ${updatedAt
            ...
            .replace(/-/g, '/')}`}</em>
          {isPublish ? (
            <div className=" rounded-xl border-2 border-green-500 px-3 text-green-500">Published</div>
          ) : (
            <div className=" rounded-xl border-2 border-gray-500 px-3 dark:text-gray-500">Draft</div>
          )}
        </div>
        <Link to={slug} title={slug} prefetch="intent">
          <h5 className="mb-2 line-clamp-3 max-h-20 overflow-hidden text-lg font-bold tracking-tight hover:underline dark:text-slate-200">
            {title}
          </h5>
        </Link>
        <p className="mb-1 line-clamp-2 h-10 max-h-10 overflow-hidden text-sm font-normal dark:text-slate-400">
          {preface}
        </p>
        <Link
          to={slug}
          title={slug}
          prefetch="intent"
          className="inline-flex items-center justify-center rounded bg-sky-700 px-4 py-2 font-bold text-white hover:bg-sky-600 focus:bg-sky-400">
          Preview
          <svg
            aria-hidden="true"
